...
var rob = function(nums) {
  ...
  for(let i=0; i<nums.length; i++) {
    ...
    if(s === e) dp[i] = nums[i] + nums[s];
    else if(s < e) {
      if(e-s === 1) dp[i] = nums[i] + Math.max(nums[s], nums[e]);
      else {
        let acc = JSON.parse(JSON.stringify(nums.slice(s, e+1)));
      ...
      let acc = JSON.parse(JSON.stringify(nums.slice(s).concat(nums.slice(0,e+1))));
      for(let i=acc.length-3; i>=0; i--) {
        acc[i] += Math.max.apply(null, acc.slice(i+2));
      }
      dp[i] = nums[i] + Math.max.apply(null, acc);
